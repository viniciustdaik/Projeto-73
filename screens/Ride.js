...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      alert("Por favor insira/digitalize um ID válido");
      ...
      if (isEligible) {
        ...
        alert(//Alert.alert todas antigamente
      ...
      if (isEligible) {
        ...
        alert("Esperamos que tenha gostado do seu passeio");
        this.setState({
          bikeAssigned: false
        });
    ...
    if (userRef.docs.length == 0) {
      ...
      userRef.docs.map(doc => {
        if (!doc.data().bike_assigned) {
          ...
          isUserEligible = false;
          alert("Finalize o passeio atual para alugar outra bicicleta");
          this.setState({
            bikeId: ""
          });
        }
  ...
  checkUserEligibilityForEndRide = async (bikeId, userId) => {
    ...
    transactionRef.docs.map(doc => {
      ...
      if (lastBikeTransaction.user_id === userId) {
        ...
        isUserEligible = false;
        alert("Essa bicicleta está alugada por outro usuário");
        this.setState({
          bikeId: ""
        });
      }
